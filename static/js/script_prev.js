...
function setupEventListeners() {
    ...
    modeToggle.on("change", function() {
        currentMode = d3.select(this).property("checked") ? 'extract_links' : 'add_node';
        modeLabel.text(currentMode === 'extract_links' ? "Extract Links Mode" : "Add Node Only Mode");
        const descText = currentMode === 'extract_links'
            ? "<strong>Current: Extract Links Mode</strong> - When a page is loaded, all its valid links are also added to the graph."
            : "<strong>Current: Add Node Only Mode</strong> - When a page is loaded, only that page is added as a node. Edges are only added if the page links *to* an already existing node.";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setupEventListeners();
    fetchGraphData(); // Load initial graph state if any (likely empty)

    // Set initial mode description based on checkbox state
    currentMode = modeToggle.property("checked") ? 'extract_links' : 'add_node';
    const initialDescText = currentMode === 'extract_links'
            ? "<strong>Current: Extract Links Mode</strong> - When a page is loaded, all its valid links are also added to the graph."
            : "<strong>Current: Add Node Only Mode</strong> - When a page is loaded, only that page is added as a node. Edges are only added if the page links *to* an already existing node.";
